...
test("MatchDao.create requires player ids", async () => {
	...
	await assert.rejects(
		() => dao.create({ player1Id: null, player2Id: "player-2", rngSeed: 1 }),
		/player1Id and player2Id are required/
	);
	assert.equal(create.calls.length, 0);
});
...
test("MatchDao.create requires player2Id", async () => {
	...
	await assert.rejects(
		() => dao.create({ player1Id: "player-1", player2Id: null, rngSeed: 1 }),
		/player1Id and player2Id are required/
	);
	assert.equal(create.calls.length, 0);
});
...
test("MatchDao.create requires rngSeed", async () => {
	...
	await assert.rejects(
		() => dao.create({ player1Id: "player-1", player2Id: "player-2", rngSeed: null }),
		/rngSeed is required/
	);
	assert.equal(create.calls.length, 0);
});
