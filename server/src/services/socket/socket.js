...
/*   Updated: 2025/11/17 16:01:15 by npatron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
function removePlayerFromRoom(io, userId, output) {
	...
	if (!room) throw new Error('Failed: invalid room');
...
export default async function socketLogic(io) {
	io.on('connection', (socket) => {
		...
		if (output == '0') {
			...
			if (output == 'error: room full') console.log('ROOM FUUUUULLLL');
		}
		socket.on('move', () => {});
